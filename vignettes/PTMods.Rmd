...
that happend after protein biosynthesis that can have a major
influence of protein function. PTMs are biologically very important
and can be identified in a high-throughput way using
mass-spectrometry.

The `PTMods` package focuses on handling such PTMs and works in
conjuction with the `r BiocStyle::Biocpkg("PSMatch")` package. It
disctributes PTMs from the Unimod database and allows to convert PTM
annotations between multiple annotation formats.
...
- [Uniprot/Swiss-Prot](https://www.uniprot.org/) protein sequence database
- [Prosite](https://prosite.expasy.org/) database of protein families and domains
