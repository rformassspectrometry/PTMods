...
test_that("convertAnnotation converts name to delta_mass", {
    # Single modification
    expect_equal(convertAnnotation("M[Oxidation]PEPTIDE", convert_to = "delta_mass"),
                 "M[+15.994915]PEPTIDE")

    # Multiple modifications
    expect_equal(convertAnnotation("M[Oxidation]EVNES[Phospho]PEK",
                                   convert_to = "delta_mass"),
                 "M[+15.994915]EVNES[+79.966331]PEK")

    # No modifications
    expect_equal(convertAnnotation("PEPTIDE", convert_to = "delta_mass"),
                 "PEPTIDE")
})

test_that("convertAnnotation converts name to unimod_id", {
    # Single modification
    expect_equal(convertAnnotation("M[Oxidation]PEPTIDE", convert_to = "unimod_id"),
                 "M[UNIMOD:35]PEPTIDE")

    # Multiple modifications
    expect_equal(convertAnnotation("C[Carbamidomethyl]PEPTIDE",
                                   convert_to = "unimod_id"),
                 "C[UNIMOD:4]PEPTIDE")
})

test_that("convertAnnotation converts delta_mass to name", {
    # Single modification
    expect_equal(convertAnnotation("M[+15.995]PEPTIDE", convert_to = "name"),
                 "M[Oxidation]PEPTIDE")

    # Multiple modifications
    expect_equal(convertAnnotation("S[+79.966]EQUENCE", convert_to = "name"),
                 "S[Phospho]EQUENCE")
})

test_that("convertAnnotation converts delta_mass to unimod_id", {
    expect_equal(convertAnnotation("M[+15.995]PEPTIDE", convert_to = "unimod_id"),
                 "M[UNIMOD:35]PEPTIDE")

    expect_equal(convertAnnotation("S[+79.966]PEPTIDE", convert_to = "unimod_id"),
                 "S[UNIMOD:21]PEPTIDE")
})

test_that("convertAnnotation converts unimod_id to name", {
    expect_equal(convertAnnotation("M[UNIMOD:35]PEPTIDE", convert_to = "name"),
                 "M[Oxidation]PEPTIDE")

    expect_equal(convertAnnotation("C[UNIMOD:4]PEPTIDE", convert_to = "name"),
...
test_that("convertAnnotation handles vectorized input", {
    ...
    result <- convertAnnotation(sequences, convert_to = "delta_mass")
...
test_that("convertAnnotation handles sequences already in target format", {
    ...
    expect_equal(convertAnnotation("M[Oxidation]PEPTIDE", convert_to = "name"),
                 "M[Oxidation]PEPTIDE")
...
test_that("convertAnnotation input validation", {
    # Non-character input
    expect_error(convertAnnotation(123, convert_to = "delta_mass"),
    ...
    expect_error(convertAnnotation("M[Oxidation]PEPTIDE", convert_to = "invalid"),
...
test_that(".detectModificationType correctly identifies modification types", {
    # Unimod ID formats
    expect_equal(unimod:::.detectModificationType("UNIMOD:35"), "unimod_id")
    expect_equal(unimod:::.detectModificationType("U:35"), "unimod_id")
    expect_equal(unimod:::.detectModificationType("unimod:35"), "unimod_id")

    # Delta mass formats
    expect_equal(unimod:::.detectModificationType("+15.995"), "delta_mass")
    expect_equal(unimod:::.detectModificationType("-17.026"), "delta_mass")
    expect_equal(unimod:::.detectModificationType("+79.966331"), "delta_mass")
    expect_equal(unimod:::.detectModificationType("+15"), "delta_mass")
...
test_that(".lookupByName finds modifications correctly", {
    ...
    expect_equal(result$unimod_id, 35)
...
test_that(".lookupByUnimodId finds modifications correctly", {
    ...
    expect_type(result, "list")
    expect_equal(result$unimod_id, 21)
...
test_that(".convertAnnotation handles single sequences", {
    ...
    expect_error(unimod:::.convertAnnotation(123, convert_to = "delta_mass"),
                 "must be a single character string")
})
